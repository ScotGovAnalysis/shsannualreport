...
`Walking` = cell_spec(`Walking`, "latex", background = case_when(`Walking_sig` == "HIGHER" ~ "#00A3A3", `Walking_sig` == "LOWER" ~ "#C3C3FF", TRUE ~ "white"))
...
`Walking` = cell_spec(`Walking`, "latex", background = case_when(`Walking_sig` == "HIGHER" ~ "#C3C3FF", `Walking_sig` == "LOWER" ~ "#00A3A3", TRUE ~ "white"))) %>%
        select(!tidyselect::all_of(significance_column_names)) %>%
        kable("latex", escape = FALSE)
```
